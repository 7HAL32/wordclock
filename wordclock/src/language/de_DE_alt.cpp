...
void Grid_de_DE_alt::setTime(int hour, int minute) { 
  ...
  for(int i = 0; i < 5; i++) {
	Led::ids[Led::getLedId(Grid_de_DE_alt::time_it_is[i])].setRGB(Config::color_fg.r, Config::color_fg.g, Config::color_fg.b);
  }

  for(int m = 0; m < 12; m++) {
	if(Grid_de_DE_alt::time_minutes[minute][m] >= 0) {
	  Led::ids[Led::getLedId(Grid_de_DE_alt::time_minutes[minute][m])].setRGB(Config::color_fg.r, Config::color_fg.g, Config::color_fg.b);
	}
  }
  ...
  for(int h = 0; h < hourLimit; h++) {   
	if(Grid_de_DE_alt::time_hours[hour][h] >= 0) {
	  Led::ids[Led::getLedId(Grid_de_DE_alt::time_hours[hour][h])].setRGB(Config::color_fg.r, Config::color_fg.g, Config::color_fg.b);
	}
  }
